...
export async function POST(req: NextRequest) {
  ...
  if (values.some(val => val === undefined || isNaN(val)) || fertilityClass === undefined || confidence === undefined) {
    return NextResponse.json({ status: 0, message: 'Missing or invalid data', error: 'Missing or invalid data' }, { status: 400 });
  }
  ...
  return NextResponse.json({ status: 1, message: 'Record created', data: record });
}
...
export async function PUT(req: NextRequest) {
  ...
  return NextResponse.json({ status: 1, message: 'Record updated', data: record });
}
